...
/**
 * O ERRO DA VERIFICAÇÃO DE SESSÃO É REGISTRADO — E O NÃO-ERRO NÃO É.
 *
 * ## O defeito original
 *
 * `loadAuthUser` desestruturava só o dado do usuário e jogava o `error` fora.
 * "Sem usuário" significa TANTO "não está logado" (estado normal) QUANTO "não deu
 * para verificar" (GoTrue fora do ar, rede, token ilegível) — e uma falha
 * transitória virava, no log, uma pessoa simplesmente deslogada, indistinguível
 * do normal.
 *
 ...
 *
 * `if (error) logger.error(...)` parece a correção óbvia e é uma armadilha: **a
 * ausência de sessão devolve erro no caminho normal**. O projeto **não tem
 * `middleware.ts`** (o gate de `/app/*` é o próprio `app/app/layout.tsx`, que
 * chama esta função), então TODO visitante deslogado passa por aqui sem sessão.
 *
 ...
 * ## O que mudou depois
 *
 * A verificação passou de `getUser()` (ida ao servidor de Auth a cada chamada)
 * para `getClaims()` (verificação LOCAL da assinatura do JWT), e com ela o que a
 * função lê: `data.claims` em vez de `data.user`, e o `sub` do claim no lugar do
 * `user.id`. Os dois defeitos acima — e os dois casos que os prendem — continuam
 * exatamente os mesmos; só a origem do dado mudou. Um dublê de `getUser` aqui não
 * seria chamado nunca.
 */

interface RespostaDeClaims {
  data: { claims: unknown };
  error: { name?: string; message: string; status?: number; code?: string } | null;
}

const resposta: { atual: RespostaDeClaims } = {
  atual: { data: { claims: null }, error: null },
};
...
vi.mock("@/lib/supabase/server", () => ({
  createClient: async () => ({
    // `getClaims` — a verificação do JWT é LOCAL agora. O que a função lê é
    // `data.claims`, e `claims: null` é o "não está logado".
    auth: { getClaims: async () => resposta.atual },
    from: () => {
...
beforeEach(() => {
  erros.length = 0;
  resposta.atual = { data: { claims: null }, error: null };
});

describe("loadAuthUser — o erro do getClaims deixa rastro", () => {
  it("falha transitória do GoTrue REGISTRA, com a classe e o código", async () => {
    ...
    expect(erros[0]!.msg).toMatch(/getClaims falhou/);
  ...
  it("visitante SEM SESSÃO não registra nada — é o estado normal, não incidente", async () => {
    // Sem cookie de sessão o `getClaims` não tem token para verificar e devolve
    // `claims: null` com um erro de sessão ausente — que é o caminho normal de
    // quem visita `/app/*` deslogado, não um incidente.
    resposta.atual = {
      data: { claims: null },
